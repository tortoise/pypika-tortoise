...
class JSON(Term):
    ...
    def _get_dict_sql(self, value: dict, **kwargs: Any) -> str:
        ...
        for k, v in value.items():
            key_sql = self._recursive_get_sql(k, **kwargs)
            value_sql = self._recursive_get_sql(v, **kwargs)
...
class Star(Field):
    ...
    def get_sql(self, ctx: SqlContext) -> str:
        if self.table and (ctx.with_namespace or self.table.alias):
            ...
            quoted_namespace = format_quotes(namespace, ctx.quote_char)
            return f"{quoted_namespace}.*"
...
class BetweenCriterion(RangeCriterion):
    ...
    def get_sql(self, ctx: SqlContext) -> str:
        ...
        sql = f"{term} BETWEEN {start} AND {end}"
        return format_alias_sql(sql, self.alias, ctx)
...
class Case(Term):
    ...
    def get_sql(self, ctx: SqlContext) -> str:
        ...
        case_items: list[str] = []
        for criterion, term in self._cases:
            when = criterion.get_sql(when_then_else_ctx)
            then = term.get_sql(when_then_else_ctx)
            case_items.append(f"WHEN {when} THEN {then}")
        cases = " ".join(case_items)
        else_ = (" ELSE " + self._else.get_sql(when_then_else_ctx)) if self._else else ""
...
class Not(Criterion):
    ...
    def get_sql(self, ctx: SqlContext) -> str:
        ...
        term = self.term.get_sql(not_ctx)
        sql = f"NOT {term}"
        return format_alias_sql(sql, self.alias, ctx)
...
class Function(Criterion):
    ...
    def get_sql(self, ctx: SqlContext) -> str:
        ...
        if self.schema is not None:
            ...
            function_sql = f"{schema}.{function_sql}"
...
class AnalyticFunction(AggregateFunction):
    ...
    def _orderby_field(self, field: Field, orient: Order | None, ctx: SqlContext) -> str:
        ...
        field_sql = field.get_sql(ctx)
        return f"{field_sql} {orient.value}"
...
class WindowFrameAnalyticFunction(AnalyticFunction):
    ...
    def get_partition_sql(self, ctx: SqlContext) -> str:
        ...
        return f"{partition_sql} {frame_sql}"
